...
class QiuBaiSpider(object):
    ...
    def run(self):
        process_list = []
        # 构造url列表
        t_url_list = Process(target=self.get_url_list)
        t_url_list.daemon = True
        t_url_list.start()
        t_url_list.join()

        # 发送请求，获取响应
        for i in range(3):
            ti_parse_url = Process(target=self.parse_url)
            process_list.append(ti_parse_url)
        # 提取数据
        ti_parse_item = Process(target=self.parse_item)
        process_list.append(ti_parse_item)
        # 保存数据
        ti_save_item = Process(target=self.save_item)
